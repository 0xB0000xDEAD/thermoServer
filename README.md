...
A RESTful API to various embedded node transmitting sensor data via MQTT protocol
...
npm run dev-debug #  debug support for docker istance
```
...
The API server can be run in a docker instance locally or in the cloud ( herouku, zeit, digital ocean etc etc) 
...
$ docker-compose up
```
this will build and run the image ad daemon
